...
import static com.locat.api.infra.aws.s3.impl.LocatS3ClientImpl.DIRECTORY_DELIMETER;
...
public final class FileUtils {
   ...
   * @param file 파일
   * @return 파일 확장자
   * @throws FileOperationException 파일 이름에 확장자가 포함되어 있지 않은 경우
   */
  public static String extractExtension(MultipartFile file) {
    final String originalFilename =
        Optional.of(file)
            ...
            .orElseThrow(
                () -> new FileOperationException(ApiExceptionType.FILE_EXTENSION_NOT_PROVIDED));
    final int lastIndexOfDot = originalFilename.lastIndexOf(".");
    if (lastIndexOfDot == -1 || lastIndexOfDot == originalFilename.length() - 1) {
      throw new FileOperationException(ApiExceptionType.FILE_EXTENSION_NOT_PROVIDED);
    }
